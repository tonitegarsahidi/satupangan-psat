    ...
    public function run(): void
    {
        ...
        foreach (config('workflow.action_types') as $i => $actionType) {
            WorkflowAction::create([
                ...
                'user_id' => $user,
                'action_time' => Carbon::now()->addMinutes($i * 10),
                'action_type' => $actionType,
                'action_target' => null,
                'description' => "Contoh action $actionType",
                'previous_status' => config('workflow.statuses')[max(0, $i-1)],
                'new_status' => config('workflow.statuses')[min($i, count(config('workflow.statuses'))-1)],
