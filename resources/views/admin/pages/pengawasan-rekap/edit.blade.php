                                        ...
                                        <div class="col-12">
                                                                ...
                                                                <td>
                                                                    <div class="form-check">
                                                                        <input class="form-check-input pengawasan-checkbox" type="checkbox"
                                                                               name="pengawasan_ids[]" value="{{ $pengawasan->id }}"
                                                                               id="pengawasan-{{ $pengawasan->id }}"
                                                                               @if(in_array($pengawasan->id, $selectedPengawasanIds ?? []))
                                                                               checked
                                                                               @endif>
                                            ...
                                            <div class="mt-3">
                                                ...
                                                <span class="text-muted ms-2">
                                                    <span id="selected-count">{{ count($selectedPengawasanIds ?? []) }}</span> data terpilih
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            $.each(response.data, function(index, pengawasan) {
                                var isChecked = false;
                                if ('{{ json_encode($selectedPengawasanIds) }}'.includes(pengawasan.id)) {
                                    isChecked = true;
                                }

                                tbody += '<tr>';
                                tbody += '<td><div class="form-check"><input class="form-check-input pengawasan-checkbox" type="checkbox" name="pengawasan_ids[]" value="' + pengawasan.id + '" id="pengawasan-' + pengawasan.id + '"' + (isChecked ? ' checked' : '') + '><label class="form-check-label" for="pengawasan-' + pengawasan.id + '"></label></div></td>';
